...
class code_parts_model extends MY_Model {
     ...
     * Handles both single and separate content modes
     * @param string $page_key The page identifier
     * @param string $device Device type: 'mobile', 'desktop', or null for auto-detect
     * @return array Array with 'content', 'content_mobile', 'content_desktop', and 'content_mode'
     */
    public function get_content($page_key, $device = null){
        $result = $this->db->select('content, content_mode, content_mobile, content_desktop')
        ...
        return [
            ...
            'content_mode' => $result->content_mode ?? 'single',
            'content_mobile' => $result->content_mobile ?? '',
            'content_desktop' => $result->content_desktop ?? ''
        ];
    }
     ...
     * @param string $page_key The page identifier
     * @param string $content The HTML content (for single mode)
     * @param array $settings Additional settings (device_visibility, display_position, content_mode, etc.)
     * @return bool
     */
    public function save($page_key, $content, $settings = []){
        ...
        // Add content mode and device-specific content
        if (isset($settings['content_mode'])) {
            $data['content_mode'] = $settings['content_mode'];
        }
        if (isset($settings['content_mobile'])) {
            $data['content_mobile'] = $settings['content_mobile'];
        }
        if (isset($settings['content_desktop'])) {
            $data['content_desktop'] = $settings['content_desktop'];
        }
        ...
        if ($existing) {
            ...
            if (!isset($data['content_mode'])) $data['content_mode'] = 'single';
    ...
    public function get_all($activeOnly = false){
        $this->db->select('id, page_key, page_name, status, content_mode, device_visibility, display_position, show_on_mobile, show_on_desktop, updated_at')
    ...
    public function get_settings($page_key){
        return $this->db->select('content_mode, device_visibility, display_position, show_on_mobile, show_on_desktop')
                        ->where('page_key', $page_key)
                        ->get($this->table)
                        ->row();
    }
