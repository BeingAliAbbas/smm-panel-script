...
class code_parts extends MX_Controller {
    ...
    public function index(){
        ...
        foreach ($code_parts as $part) {
            $code_parts_settings[$part->page_key] = [
                'content_mode' => isset($part->content_mode) ? $part->content_mode : $defaults['content_mode'],
                'device_visibility' => isset($part->device_visibility) ? $part->device_visibility : $defaults['device_visibility'],
                'display_position' => isset($part->display_position) ? $part->display_position : $defaults['display_position'],
                'show_on_mobile' => isset($part->show_on_mobile) ? (int)$part->show_on_mobile : $defaults['show_on_mobile'],
                'show_on_desktop' => isset($part->show_on_desktop) ? (int)$part->show_on_desktop : $defaults['show_on_desktop'],
            ];
        }
    ...
    public function ajax_save() {
        ...
        // Get content mode
        $content_mode = $this->input->post('content_mode', true) ?: 'single';
        
        // Get device-specific content if in separate mode
        $content_mobile = '';
        $content_desktop = '';
        if ($content_mode === 'separate') {
            $content_mobile = $this->input->post('content_mobile', false); // false to allow HTML
            $content_desktop = $this->input->post('content_desktop', false); // false to allow HTML
            
            // Sanitize device-specific content
            $content_mobile = $this->sanitize_html($content_mobile);
            $content_desktop = $this->sanitize_html($content_desktop);
        }
        ...
        $settings = [
            ...
            'show_on_mobile' => ($this->input->post('show_on_mobile') == '1') ? 1 : 0,
            'show_on_desktop' => ($this->input->post('show_on_desktop') == '1') ? 1 : 0,
        ];
