...
class balance_logs extends MX_Controller {
	...
	public function index(){
		...
		if (get_role('admin') || get_role('supporter')) {
			...
			$filters['action_type']  = get('action_type');
			$filters['related_type'] = get('related_type');
	...
	public function export_csv(){
		...
		$balance_logs = $this->model->get_balance_logs_for_export($filters);

		// Set headers for CSV download
		header('Content-Type: text/csv; charset=utf-8');
		header('Content-Disposition: attachment; filename=balance_logs_'.date('Y-m-d_H-i-s').'.csv');
	...
	/**
	 * View balance log details (modal)
	 */
	public function view_details($ids = ""){
		if (!get_role('admin') && !get_role('supporter')) {
			redirect(cn());
		}
		$balance_log = $this->model->get_balance_log_details($ids);
		$data = array(
			"module"      => $this->module,
