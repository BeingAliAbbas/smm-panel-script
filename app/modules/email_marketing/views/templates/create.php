<div class="modal-header">
  <h4 class="modal-title"><i class="fe fe-file-text"></i> Create New Email Template</h4>
  <button type="button" class="close" data-dismiss="modal" aria-label="Close"></button>
</div>

<form class="actionForm" action="<?php echo cn($module . '/ajax_template_create'); ?>" method="POST">
  <div class="modal-body">
    
    <div class="form-group">
      <label>Template Name <span class="text-danger">*</span></label>
      <input type="text" class="form-control" name="name" placeholder="e.g., Welcome Email" required>
    </div>
    
    <div class="form-group">
      <label>Email Subject <span class="text-danger">*</span></label>
      <input type="text" class="form-control" name="subject" placeholder="e.g., Welcome to {site_name}!" required>
    ...
    <div class="form-group">
      ...
      <textarea class="form-control" name="body" id="email_body" rows="15" required></textarea>
  ...
  <div class="modal-footer">
    ...
    <button type="submit" class="btn btn-primary">
      <i class="fe fe-plus"></i> Create Template
    </button>
  </div>
</form>
    ...
    $.ajax({
      ...
      success: function(response){
        if(response.status == 'success'){
          ...
          setTimeout(function(){
            window.location.href = '<?php echo cn($module . '/templates'); ?>';
          }, 1000);
        } else {
          show_message(response.message, 'error');
          $form.find('button[type="submit"]').prop('disabled', false).html('<i class="fe fe-plus"></i> Create Template');
        }
      },
      error: function(){
