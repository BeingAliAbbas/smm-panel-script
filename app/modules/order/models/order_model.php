...
<<<<<<< HEAD
	 ...
	 */
	public function get_bulk_orders_full($status) {
		$this->db->select("id, ids, uid, service_id, charge, quantity, status, formal_charge, profit, created");
		$this->db->from($this->tb_order);
		$this->db->where("status", $status);
		$query = $this->db->get();
...
<<<<<<< HEAD
	 ...
	 * @param string $link Order link
	 * @return object|false Order object if duplicate found, false otherwise
	 */
	public function check_duplicate_order($uid, $link) {
		$this->db->select("id, ids, service_id, link, status, created");
		$this->db->from($this->tb_order);
		$this->db->where("uid", $uid);
		$this->db->where("link", $link);
		// Only check for orders that are still being processed
		$this->db->where_in("status", ['pending', 'processing', 'inprogress']);
