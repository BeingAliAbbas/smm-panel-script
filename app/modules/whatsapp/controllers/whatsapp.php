...
class Whatsapp extends MX_Controller {
    ...
    /**
     * Toggle WhatsApp messaging globally (admin only)
     */
    public function toggle_messaging() {
        if (!get_role('admin')) {
        ...
        if ($this->model->toggle_messaging($enabled)) {
            $status_text = $enabled ? 'enabled' : 'disabled';
            ms([
                'status' => 'success',
                'message' => "WhatsApp messaging has been {$status_text}"
            ]);
        } else {
            ms([
                'status' => 'error',
