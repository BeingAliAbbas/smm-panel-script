...
class Whatsapp_model extends MY_Model {
    ...
    protected $messaging_enabled = true;
    ...
    private function _load_config() {
        try {
            ...
            if ($config && !empty($config->base_url) && !empty($config->api_key)) {
                ...
                $this->messaging_enabled = isset($config->messaging_enabled) ? (bool)$config->messaging_enabled : true;
    ...
    /**
     * Check if WhatsApp messaging is globally enabled
     */
    ...
    public function send_message($phone_number, $message) {
        ...
        if (!$this->messaging_enabled) {
            // Silently skip sending when disabled - no errors, no logs, no retries
            return ['success' => true, 'skipped' => true, 'message' => 'WhatsApp messaging is globally disabled'];
        }
    ...
    /**
     * Toggle WhatsApp messaging globally
     * @param bool $enabled - true to enable, false to disable
     * @return bool
     */
    public function toggle_messaging($enabled) {
        if (!$this->db->table_exists('whatsapp_config')) {
            return false;
        }
        ...
        if ($existing) {
            $this->db->where('id', $existing->id);
            $result = $this->db->update('whatsapp_config', ['messaging_enabled' => $enabled ? 1 : 0]);
            
            // Reload config after updating
            if ($result) {
                $this->messaging_enabled = (bool)$enabled;
            }
