...
<div class="row">
    ...
    <div class="col-md-4">
        ...
        <!-- Global Messaging Toggle Card (Admin Only) -->
        <?php if ($is_configured): 
            $messaging_enabled = isset($config->messaging_enabled) ? (bool)$config->messaging_enabled : true;
        ?>
        <div class="card mt-4">
            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="fe fe-power text-warning"></i> <?=lang("Global Messaging Control")?></h5>
                <span class="badge <?=$messaging_enabled ? 'bg-success' : 'bg-danger'?>">
                    <?=$messaging_enabled ? lang('Enabled') : lang('Disabled')?>
                </span>
            </div>
            <div class="card-body">
                <div class="alert alert-info">
                    <i class="fe fe-info"></i> 
                    <strong><?=lang("Note")?>:</strong> <?=lang("Use this toggle to enable or disable ALL WhatsApp messaging globally. When disabled, the system will silently skip all WhatsApp message sending attempts without errors or retries.")?>
                </div>
                <div class="form-group">
                    <div class="custom-control custom-switch">
                        <input type="checkbox" class="custom-control-input" id="messagingToggle" 
                               <?=$messaging_enabled ? 'checked' : ''?>>
                        <label class="custom-control-label" for="messagingToggle">
                            <strong><?=lang("Enable WhatsApp Messaging")?></strong>
                        </label>
                    </div>
                    <small class="text-muted">
                        <?=lang("When disabled, no WhatsApp messages will be sent from any module in the system.")?>
                    </small>
                </div>
                <button type="button" class="btn btn-warning btn-block" id="saveMessagingToggle">
                    <i class="fe fe-save"></i> <?=lang("Save Messaging Settings")?>
                </button>
            </div>
        </div>
        <?php endif; ?>
...
$(document).ready(function() {
    ...
    // Save messaging toggle
    $('#saveMessagingToggle').on('click', function() {
        var $btn = $(this);
        var enabled = $('#messagingToggle').is(':checked');
        ...
        $.ajax({
            url: '<?=cn("whatsapp/toggle_messaging")?>',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function(response) {
                $btn.prop('disabled', false).html('<i class="fe fe-save"></i> <?=lang("Save Messaging Settings")?>');
                if (response.status === 'success') {
                    showMessage(response.message, 'success');
                    // Update badge
                    var $badge = $('.card-header .badge');
                    $badge.removeClass('bg-success bg-danger');
                    $badge.addClass(enabled ? 'bg-success' : 'bg-danger');
                    $badge.text(enabled ? '<?=lang("Enabled")?>' : '<?=lang("Disabled")?>');
                } else {
                    showMessage(response.message || '<?=lang("Failed to save")?>', 'error');
                }
            },
            error: function() {
                $btn.prop('disabled', false).html('<i class="fe fe-save"></i> <?=lang("Save Messaging Settings")?>');
